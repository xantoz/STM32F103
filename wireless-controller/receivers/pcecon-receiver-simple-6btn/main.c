...
void pceCon_IRQHandler()
{
    const uint32_t ENABLE_Msk = (0x1 << ENABLE_PortPin.pin);
    const uint32_t SELECT_Msk = (0x1 << SELECT_PortPin.pin);
    static uint32_t enable_cntr = 0;
    static uint32_t select_cntr = 0;
    ...
    {
        GPIOB.ODR &= ~(0xf << OUTPUT_Pos);
    }

    if (EXTI.PR & SELECT_Msk)
    {
        bool select_now = GPIO_read(&SELECT_PortPin);

        // count negative select flanks
        if (select_prev == true && select_now == false)
        {
            if (select_cntr++ >= 3)
                enabled = false;
        }
